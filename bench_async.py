...
SEED = 42              # 固定随机种子，保证 sync/async 输入完全一致
...
def example_sync(prompts, sampling_params):
    """串行模式示例（默认）"""
    print("=" * 60)
    print("串行模式示例")
    print("=" * 60)
    ...
    try:
        ...
        run_warmup(llm)
        # 使用与异步模式相同的一份 prompts
        run_bandwidth(llm, prompts, sampling_params)
        llm.save_timing_log("timing_sync.json")
    finally:
        # 确保释放进程/显存资源，便于后续重新实例化
        llm.exit()


def example_async(prompts, sampling_params):
    """异步流水线模式示例"""
    print("\n" + "=" * 60)
    print("异步流水线模式示例")
    print("=" * 60)
    ...
    try:
        # 先执行一次轻量 warmup，避免首轮冷启动抖动
        run_warmup(llm)
        # 使用与串行模式相同的一份 prompts
        run_bandwidth(llm, prompts, sampling_params)
        llm.save_timing_log("timing_async.json")
    finally:
        # 确保释放资源，防止与后续实例冲突
        llm.exit()
...
if __name__ == "__main__":
    ...
    # 串行模式（与异步共用同一份 prompts）
    example_sync(prompts, sampling_params)

    # 异步模式（与串行共用同一份 prompts）
    example_async(prompts, sampling_params)