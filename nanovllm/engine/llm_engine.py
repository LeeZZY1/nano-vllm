...
class LLMEngine:
    ...
    def _step_sync(self):
        ...
        print(f"[sync main]   CUDA stream id={stream.stream_id}, ptr={stream.cuda_stream}")
        ...
        # 记录timing
        if not hasattr(self, '_timing_log'):
            self._timing_log = []
        self._timing_log.append({
            "mode": "sync",
            "is_prefill": is_prefill,
            "num_seqs": len(seqs),
            "schedule_ms": (t1 - t0) * 1000,
            "gpu_ms": (t2 - t1) * 1000,
        ...
        if self.pending_batch is not None:
            ...
            if token_ids is not None:
                ...
                self._timing_log.append({
                    ...
                    "wait_ms": (t1 - t5) * 1000,
                    "postprocess_ms": (t2 - t1) * 1000,
                    "schedule_ms": (t5 - t4) * 1000,
                    "run_async_ms": (t6 - t5) * 1000,
                })
    ...
    def save_timing_log(self, path: str):
        if hasattr(self, '_timing_log') and self._timing_log:
            with open(path, 'w') as f:
                json.dump(self._timing_log, f, indent=2)
            print(f"[timing] 已保存 {len(self._timing_log)} 条记录到 {path}")
        else:
            print(f"[timing] 无数据可保存")
